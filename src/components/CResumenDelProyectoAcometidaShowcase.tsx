...
import { CResumenDelProyectoAcometida } from '../models/CResumenDelProyectoAcometida'
import type { ResumenItem } from '../models/CResumenDelProyectoAcometida'
...
class ShowcaseAcometida extends CResumenDelProyectoAcometida {
...
const DEFAULT_ITEMS: ResumenItem[] = [
  ['Cable de acometida FO 2 hilos',  'FO-ACOM-2H', '40 M'],
  ['Conector SC/APC',                'CONN-SC',    '4 PZA'],
  ['Roseta óptica interior',         'ROS-OPT',    '1 PZA'],
  ['Canaleta plástica 20×12',        'CAN-20',     '8 M'],
  ['Caja de paso exterior',          'CAJA-EXT',   '1 PZA'],
]
  ...
  return (
    <Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Sello GIS «RESUMEN DE PROYECTO» (acometida FO) — tabla dinámica (2 + n) × 4.
        Color azul (vs rojo de CResumenDelProyecto). Row 1 colorea cols 2, 3 y 4.
        Cols 1,3 fila-1 → nombre de enlace GIS (stub Fase 5).
      </Typography>
              ...
              return (
                <TableRow key={r}>
                  {Array.from({ length: COLS }, (_, ci) => {
                    ...
                    const textColor = color ?? (isTitle || isHeader ? '#1565c0' : '#212121')
                    return (
                      <TableCell
                        key={c}
                        sx={{
      ...
      <Typography variant="subtitle2" sx={{ mb: 1, color: '#263238' }}>
        Materiales de acometida (editable)
      </Typography>
      ...
      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
        <Chip label="Azul = título + celdas coloreadas (o_color_linea=[0,0,1])" size="small" sx={{ bgcolor: '#e3f2fd', fontSize: 10 }} />
        <Chip label="Índigo = fila encabezados" size="small" sx={{ bgcolor: '#e8eaf6', fontSize: 10 }} />
