/**
 * Migracion de: c_placa_fosc350c.magik
 * Clase Magik:  c_placa_fosc350c
...
class MockSymbolDataSource implements SymbolDataSource {
  private names = ['placa_fosc 350c', 'SYM_CIRCLE', 'SYM_SQUARE'];
  ...
  async renderSymbol(
    ...
    if (name === 'placa_fosc 350c') {
      base.elements.push({ type: 'rect', x: 20, y: 30, w: 60, h: 40, fill });
      return base;
    }
...
export class PlacaFosc350c {
  name: string = 'placa_fosc 350c';
...
export function PlacaFosc350cUI() {
  const dataSource = useMemo(() => new MockSymbolDataSource(), []);
  const placa = useMemo(() => new PlacaFosc350c(dataSource), [dataSource]);

  const [names, setNames] = useState<string[]>([]);
  const [selected, setSelected] = useState('placa_fosc 350c');
  ...
  }, [placa, selected, color, angle, flip, mirror]);
  ...
  return (
    <div style={s.wrapper}>
      <div style={s.title}>c_placa_fosc350c</div>
...
export default PlacaFosc350cUI;
