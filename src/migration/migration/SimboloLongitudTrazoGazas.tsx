/**
 * Migracion de: c_simbolo_longitud_trazo_gazas.magik
 * Clase Magik:  c_simbolo_longitud_trazo_gazas
 * Metodos:      defined_attributes, symbol_names, draw_content_on
 *
 * Intencion:
...
class MockSymbolDataSource implements SymbolDataSource {
  private names = ['longitud trazo gazas', 'SYM_CIRCLE', 'SYM_SQUARE'];
  ...
  async renderSymbol(
    ...
    if (name === 'longitud trazo gazas') {
      base.elements.push({ type: 'rect', x: 14, y: 38, w: 72, h: 24, fill });
      return base;
    }
...
export class SimboloLongitudTrazoGazas {
  name: string = 'longitud trazo gazas';
  ...
  async definedAttributes(base: LayoutAttributeDefinition[] = []): Promise<LayoutAttributeDefinition[]> {
    ...
    attribs.push({
      name: 'name',
      type: 'string',
      description: 'Longitud trazo gazas',
      defaultValue: 'longitud trazo gazas',
      enumMethod: 'symbolNames',
    });
...
export function SimboloLongitudTrazoGazasUI() {
  const dataSource = useMemo(() => new MockSymbolDataSource(), []);
  const simbolo = useMemo(() => new SimboloLongitudTrazoGazas(dataSource), [dataSource]);

  const [names, setNames] = useState<string[]>([]);
  const [selected, setSelected] = useState('longitud trazo gazas');
  ...
  useEffect(() => {
    let mounted = true;
    simbolo.name = selected || '';
    simbolo.colour = color;
    simbolo.angle = angle;
    simbolo.flip = flip;
    simbolo.mirror = mirror;

    simbolo.drawContentOn().then(draw => {
  ...
  }, [simbolo, selected, color, angle, flip, mirror]);

  const render = result.render;
  const transform = buildTransform(angle, flip, mirror);
  const useImage = selected === 'longitud trazo gazas';
  const imageTransform = `rotate(${angle}deg) scaleX(${mirror ? -1 : 1}) scaleY(${flip ? -1 : 1})`;

  return (
    <div style={s.wrapper}>
      <div style={s.title}>c_simbolo_longitud_trazo_gazas</div>
      ...
      {useImage ? (
        <div style={s.previewBox}>
          <img
            src={simboloLongitudTrazoGazas}
            alt="simbolo longitud trazo gazas"
            style={{ ...s.image, transform: imageTransform }}
          />
          <div style={s.meta}>Color no aplica a bitmap.</div>
        </div>
      ) : result.status !== 'ok' ? (
...
const s: Record<string, React.CSSProperties> = {
  ...
  previewBox: { display: 'flex', flexDirection: 'column', gap: 6 },
  image   : { width: 140, height: 140, objectFit: 'contain', border: '1px solid #ddd', borderRadius: 6, background: '#f7f7f7', transformOrigin: '50% 50%' },
};

export default SimboloLongitudTrazoGazasUI;
